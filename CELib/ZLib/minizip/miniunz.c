	...
	const char* dirname;
{
    ...
    // 2005-08-17 bsc - mkdir gibt's nicht unter windows ce "Application/Wavelink/"

    wchar_t sz[255];
	wchar_t szWL[255];
	mbstowcs(szWL, "Application/Wavelink/", 255);
    mbstowcs(sz, dirname, 255);

	//MessageBox(NULL, sz , L"sz1", MB_OK);
	//MessageBox(NULL, szWL , L"sz2", MB_OK);

	if(wcscmp( sz, szWL ) == 0)
	{
		
		CreateDirectory(L"\\Application\\Wavelink",NULL);
		CreateDirectory(L"\\Application\\Wavelink\\TelnetCE",NULL);
		CreateDirectory(L"\\Application\\Wavelink\\TelnetCE\\config",NULL);
		CreateDirectory(L"\\Application\\Wavelink\\TelnetCE\\Install",NULL);

		//MessageBox(NULL, L"Wavelink Files created" , L"Create", MB_OK);

		return TRUE;

	}
	else
		return CreateDirectory(sz, NULL);
  ...
  while (1)
    {
      ...
      hold = *p;
      *p = 0;
	  if ((mymkdir(buffer) == 0))// rku
      //if ((mymkdir(buffer) == -1))// && (errno == ENOENT)) bsc
        {
		  /*LPVOID lpMsgBuf;
		  FormatMessage( 
