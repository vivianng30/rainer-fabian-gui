...
LONG CSerialEx::StopListener (DWORD dwTimeout)
{
	...
	if (m_hThread)
	{
		...
		//int iTest=CeGetThreadPriority(m_hThread);
		...
		DEBUGMSG(TRUE, (TEXT("CSerialEx::StopListener\r\n")));
	}
...
DWORD CSerialEx::ThreadProc (void)
{
	...
	//BOOL bRes=CeSetThreadPriority(m_hThread,103);
	...
	do
	{
		...
		{
			/*if(ERROR_ALREADY_INITIALIZED==m_lLastError)
			{
				int iStop=0;
			}*/
			return m_lLastError;
 ...
 //    /*
 //     * m_dwThreadPrio wird gelesen aus der Registry:
 //     *
 //     * Key: HKLM\Software\AcuLink
 //     *
 //     * DWORD MainThreadPrio                (Default: 60)
 //     * DWORD SerialThreadPrio            (Default: 60)
 //    */

 //    CeSetThreadPriority(m_hThread, m_dwThreadPrio);

 //    // setze die timeouts der seriellen Schnittstelle (COMMTIMEOUTS)

 //    /*    siehe: http://msdn.microsoft.com/en-us/library/windows/desktop/aa363190(v=vs.85).aspx
 ...
 //    // 0 wird auch des öfteren empfohlen, aber dann wartet ReadFile überhaupt nicht mehr, Folge: weißer Bildschirm
 //    timeouts.ReadTotalTimeoutMultiplier = MAXDWORD;

 //    timeouts.ReadTotalTimeoutConstant = 2000; // unit: ms
 //    timeouts.WriteTotalTimeoutMultiplier = 0;
 //    timeouts.WriteTotalTimeoutConstant = 0;

 //    if (!SetCommTimeouts(m_hFile, &timeouts))
 //       return -1; // Error setting time-outs.

 //    do
 //    {
 //        do {
 //            if (ReadFile(m_hFile, &chByte, 1, &dwRead, NULL))
 //            {
 //                if (dwRead > 0)
 //                {
 //                    // auf gehts in die Zustandsmaschine des Parsers
 //                    this->SmCommPort(chByte);
 //                }
 //            }
 //            else
 //            {
