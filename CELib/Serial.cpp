...
// die Makros _RPTF und ASSERTE sind nicht verfügbar in Compact Framework
// #include "crtdbg.h"
...
LONG CSerial::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	...
	HangParam[0]=400;
	HangParam[1]=403;
	if(!DeviceIoControl(m_hFile, IOCTL_COM_SET_HANGPARAM,HangParam, sizeof(HangParam), NULL, 0 , NULL, NULL))
	{
		//m_lLastError = ::GetLastError();
	}
	...
	BOOL test=  GetCommProperties(m_hFile,&commProp);

	UINT max=commProp.dwMaxBaud;
	int iStop=0;*/
...
LONG CSerial::Write (const void* pData, size_t iLen, DWORD* pdwWritten, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	ASSERT(!lpOverlapped || pdwWritten);
