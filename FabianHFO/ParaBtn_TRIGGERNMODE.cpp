// ParaBtn_TRIGGERNMODE.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CParaBtn_TRIGGERNMODE, CButton)
	//{{AFX_MSG_MAP(CParaBtn_TRIGGERNMODE)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_KEYDOWN()
...
void CParaBtn_TRIGGERNMODE::Draw(int nState)
{
	if(!m_bDisableText)
	{
		//if(g_hf31AcuBold )
		{
			...
			if(m_v.iValue==MAXRANGE_TRIGGER_OFF)
			{
			...
			else
			{
				...
				else
				{
					...
					if(m_v.iValue==MAXRANGE_TRIGGER_OFF)//off
					{
						//DrawText(m_hDC,getModel()->GetLanguageString(IDS_TXT_SENS_HIGH),-1,&rcUnit,DT_BOTTOM|DT_SINGLELINE|DT_CENTER);
					}
