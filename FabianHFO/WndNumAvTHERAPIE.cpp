// WndNumAvTHERAPIE.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CWndNumAvTHERAPIE, CWnd)

CWndNumAvTHERAPIE::CWndNumAvTHERAPIE():
CWndNumeric()
{
	...
	m_szName3=getModel()->GetLanguageString(IDS_PARA_FLOW);
	...
	if(getModel()->getCONFIG()->GetVentRange()==PEDIATRIC)
		m_iOffset=421;
	else
		m_iOffset=417;
...
BEGIN_MESSAGE_MAP(CWndNumAvTHERAPIE, CWnd)
...
// CWndNumAvTHERAPIE message handlers
//************************************
// Method:    OnDestroy
// FullName:  CWndNumAvTHERAPIE::OnDestroy
// Access:    protected 
// Returns:   void
// Qualifier:
//
...
//************************************
void CWndNumAvTHERAPIE::OnDestroy() 
{
	StopFlowThread();
...
// 
// **************************************************************************
bool CWndNumAvTHERAPIE::DrawData(bool bData, bool bFrames, bool bText, bool bLimits, bool bFlowmeter)
{
	...
	if(bFlowmeter && false==m_bGraphCursor)
	{
		...
		else
		{
			if(m_pcFlowmeterTherapy)
				m_pcFlowmeterTherapy->Draw(hdcMem,105,191);
		}
		...
		m_valueFlowExsp=m_iOffset-(iExpFlow1/153);
	...
	if(bData)
	{
		...
		if(false==m_bGraphCursor)
		{
			...
			if(getModel()->getDATAHANDLER()->GetExpFlowData()>=5000)
			{
			...
			{
				wsprintf(psz,_T("%0.1f"), CTlsFloat::Round(((double)getModel()->getDATAHANDLER()->GetExpFlowData())/1000, 1));
			}
...
// 
// **************************************************************************
bool CWndNumAvTHERAPIE::DrawFrames(CDC* pDC)
{
	HDC hdc = *pDC;
	...
	else
	{
		...
		{
			if(m_pcFlowmeterTherapy)
				m_pcFlowmeterTherapy->Draw(hdc,105,191);
		}
...
// 
// **************************************************************************
bool CWndNumAvTHERAPIE::DrawStaticText(CDC* pDC)
{
	HDC hdc = *pDC;
	...
	sz2 = pDC->GetTextExtent(m_szName4);
...
// 
// **************************************************************************
bool CWndNumAvTHERAPIE::DrawLimits(CDC* pDC)
{
...
// 
// **************************************************************************
void CWndNumAvTHERAPIE::StartFlowThread(void)
{
	startThread();
...
// FullName:  CWndNumAvTHERAPIE::StopFlowThread
// Access:    protected 
// Returns:   void
// Qualifier:
// Parameter: void
...
// 2015/06/19: checked for correct closing of thread
//************************************
void CWndNumAvTHERAPIE::StopFlowThread( void )
{
	if(doThread())
	{
		stopThread();
		...
		{
			theApp.getLog()->WriteLine(_T("#THR:038a"));
			if(!TerminateThread(m_pcwtFlowThread,0))
			{
				theApp.getLog()->WriteLine(_T("#THR:038b"));
			}
		}
	}
...
bool CWndNumAvTHERAPIE::doThread()
{
	bool bRes=false;
...
void CWndNumAvTHERAPIE::startThread()
{
	EnterCriticalSection(&csDoThread);
	m_bDoThread=true;
	LeaveCriticalSection(&csDoThread);
}
void CWndNumAvTHERAPIE::stopThread()
{
	EnterCriticalSection(&csDoThread);
	m_bDoThread=false;
	LeaveCriticalSection(&csDoThread);
...
{
	((CWndNumAvTHERAPIE*)pc)->DoFlow();
	return TRUE;
}

//************************************
// Method:    DoFlow
// FullName:  CWndNumAvTHERAPIE::DoFlow
// Access:    protected 
// Returns:   DWORD
// Qualifier:
// Parameter: void
...
// 2015/06/19: checked for correct closing of thread
//************************************
DWORD CWndNumAvTHERAPIE::DoFlow(void) 
{
	...
	theApp.getLog()->WriteLine(_T("#THR:038"));
