// WndSubSettingsBackupSound.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CWndSubSettingsBackupSound, CWnd)
	ON_WM_DESTROY()
...
void CWndSubSettingsBackupSound::Initialize()
{
	...
	m_szSetting = getModel()->GetLanguageString(IDS_ALARM_BACKUP_ACTIVE);
	//m_szSetting = getModel()->GetLanguageString(IDS_PARA_BACKUP)
	...
	case WM_KEYDOWN:
		{
			...
			else if(pMsg->wParam==VK_DOWN)
			{
				if(m_slider->GetPos()!=0)
				{
					m_slider->SetPos( 0 );
					m_bBeep=false;
					//getModel()->getCONFIG()->SetPlayBackupSound(m_bBeep);
				}
