...
BEGIN_MESSAGE_MAP(CParaBtn_CPAP, CButton)
	//{{AFX_MSG_MAP(CParaBtn_CPAP)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_KEYDOWN()
		...
		else if(nChar==m_kUp)	
		{
			if(m_bDepressed)
			{
				if(m_v.iValue<m_v.iUpperLimit)
				{
					...
					{
						fPInsp = getModel()->getDATAHANDLER()->PARADATA()->GetPManualNMODEPara();

						if(dTemp>fPInsp-DIFF_PEEP_PINSP)
						{
							m_bLimited=true;
							if(GetParent())
								GetParent()->PostMessage(WM_CPAP_LIMITED);
						}
						else
						{
							/*m_v.iValue=dTemp;*/

							if(dTemp>getModel()->getDATAHANDLER()->GetCurrentCPAPMaxKey())
							{
								if(m_bKeyValueAccepted==false)
								{
									...
									m_bDrawKey=true;
									if(GetParent())
										GetParent()->PostMessage(WM_CPAP_SETKEY,m_bKeyBeep);
									m_bKeyBeep=FALSE;
								}
								else
								{
					...
					{
						fPInsp = getModel()->getDATAHANDLER()->PARADATA()->GetPManualCPAPPara();
					...
					{
						fPInsp = getModel()->getDATAHANDLER()->PRESET()->GetPManualNMODEPara();
					...
					{
						fPInsp = getModel()->getDATAHANDLER()->PRESET()->GetPManualCPAPPara();
