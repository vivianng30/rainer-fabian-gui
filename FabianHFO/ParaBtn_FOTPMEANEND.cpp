// ParaBtn_FOTPMEANEND.cpp : implementation file
//
...
CParaBtn_FOTPMEANEND::CParaBtn_FOTPMEANEND(BTN btn, int nOffset,bool bScrollOver):
...
/**********************************************************************************************//**
 * Finalizes an instance of the CParaBtn_FOTPMEANEND class
 *
 * \author	Rainer Kühner
 * \date	22.02.2018
 **************************************************************************************************/

CParaBtn_FOTPMEANEND::~CParaBtn_FOTPMEANEND()
{
}


BEGIN_MESSAGE_MAP(CParaBtn_FOTPMEANEND, CButton)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_KEYDOWN()
...
/**********************************************************************************************//**
 * CParaBtn_FOTPMEANEND message handlers
 *
 * \author	Rainer Kühner
 * \date	22.02.2018
 *
...
void CParaBtn_FOTPMEANEND::Draw(int nState)
{
	if(!m_bDisableText)
	{
...
void CParaBtn_FOTPMEANEND::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if( m_bLMouseButtonDown )
	{
		if(nChar==m_kDown)	
		{
			if(m_bDepressed)
			{
				...
				{
					WORD iLowerLimit=getModel()->getDATAHANDLER()->PARADATA()->getFOThfo_PMEANSTARTPara()+getModel()->getCONFIG()->getDiffPmeanFOT();
					int dTemp = m_v.iValue-10;
					if(dTemp<iLowerLimit)
					{
						m_bLimited=true;
						if(GetParent())
							GetParent()->PostMessage(WM_FOT_PMEANEND_LIMITED);
					}
					else
					{
						m_v.iValue=dTemp;
						...
						if(GetParent())
							GetParent()->PostMessage(WM_FOT_PMEANEND_CHANGED);
					}
				}
		...
		else if(nChar==m_kUp)	
		{
			if(m_bDepressed)
			{
				...
				if(m_v.iValue<m_v.iUpperLimit)
				{
					...
					if(iTemp>getModel()->getDATAHANDLER()->PARADATA()->GetHFPMeanMaxKey())
					{
						if(m_bKeyValueAccepted==false)
						{
							...
							m_bDrawKey=true;
							if(GetParent())
								GetParent()->PostMessage(WM_PMEANEND_SETKEY,m_bKeyBeep);
							m_bKeyBeep=FALSE;
						}
						else
						{
