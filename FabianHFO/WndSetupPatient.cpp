// WndSetupPatient.cpp: Implementierungsdatei
//
...
CWndSetupPatient::CWndSetupPatient()
{
	...
	m_pcwtDeleteThread=NULL;
	m_bDoDeleteThread=false;*/
...
BEGIN_MESSAGE_MAP(CWndSetupPatient, CWnd)
	//ON_WM_PAINT()
	ON_WM_DESTROY()
	ON_WM_SETFOCUS()
	ON_BN_CLICKED(IDC_BTN_SETUP_TODAY, &CWndSetupPatient::OnBnClickedToday)
	ON_EN_SETFOCUS(IDC_EDIT_SETUP_NAME, &CWndSetupPatient::OnSetFocusEditName)
	ON_EN_SETFOCUS(IDC_EDIT_SETUP_FIRSTNAME, &CWndSetupPatient::OnSetFocusEditFirtsName)
	ON_EN_SETFOCUS(IDC_EDIT_SETUP_PERSID, &CWndSetupPatient::OnSetFocusEditPersID)
	ON_EN_SETFOCUS(IDC_EDIT_SETUP_BIRTHDAY, &CWndSetupPatient::OnSetFocusEditBirthDay)
	ON_EN_SETFOCUS(IDC_EDIT_SETUP_BIRTHMONTH, &CWndSetupPatient::OnSetFocusEditBirthMonth)
	ON_EN_SETFOCUS(IDC_EDIT_SETUP_BIRTHYEAR, &CWndSetupPatient::OnSetFocusEditBirthYear)
	ON_EN_SETFOCUS(IDC_EDIT_SETUP_WEIGHT, &CWndSetupPatient::OnSetFocusEditWeight)
...
void CWndSetupPatient::Initialize()
{
	...
	m_pEditRemark->Create(WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_BORDER,
...
void CWndSetupPatient::Show(bool bShow)
{
	if(bShow)
	{
		...
		//SetWindowPos(NULL,0,0,m_lX,m_lY,SWP_NOSIZE|SWP_NOMOVE|SWP_SHOWWINDOW);
	...
	case WM_INVALID_DATE:
		{
			if(AfxGetApp())
				AfxGetApp()->GetMainWnd()->PostMessage(WM_PIF_SIGNAL);
			//m_pEditBirthDay->SetWindowText(_T(""));
			//m_pEditBirthMonth->SetWindowText(_T(""));
			//m_pEditBirthYear->SetWindowText(_T(""));
			return 1;
		}
		break;
	case WM_INVALID_DAY:
	...
	case WM_PATIENT_FOCUS_RIGHT:
		{
			...
			/*DWORD dwSel = m_pEdit->GetSel();
			m_pEdit->SetSel(HIWORD(dwSel), -1);*/
...
void CWndSetupPatient::OnSetFocus(CWnd* pOldWnd)
{
	...
	/*if(m_pEditName)
	{
		if(m_pEditName->IsWindowVisible())
		{
			m_pEditName->SetFocus();
			m_pcWndKeyboard->SetEditWnd(m_pEditName,KR_NONE,IDC_EDIT_SETUP_NAME);
		}
