// WndSubSettingsPminDelay.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CWndSubSettingsPminDelay, CWnd)

/**********************************************************************************************//**
 * Initializes a new instance of the CWndSubSettingsPminDelay class
 *
 * \author	Rainer Kühner
 * \date	26.02.2018
 *
...
CWndSubSettingsPminDelay::CWndSubSettingsPminDelay(UINT viewFlag):
...
/**********************************************************************************************//**
 * Finalizes an instance of the CWndSubSettingsPminDelay class
 *
 * \author	Rainer Kühner
 * \date	26.02.2018
 **************************************************************************************************/

CWndSubSettingsPminDelay::~CWndSubSettingsPminDelay()
{
...
BEGIN_MESSAGE_MAP(CWndSubSettingsPminDelay, CWnd)
	ON_WM_DESTROY()
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BTN_SETUP_YEAR, &CWndSubSettingsPminDelay::OnBnClickedValue)
END_MESSAGE_MAP()

/**********************************************************************************************//**
 * CWndSubSettingsPminDelay message handlers
 *
 * \author	Rainer Kühner
 * \date	26.02.2018
 **************************************************************************************************/

void CWndSubSettingsPminDelay::Initialize()
{
	CClientDC dc(this);

	m_iValue=getModel()->getCONFIG()->GetCurPminAlarmDelay();
	CStringW strVal;
	...
	m_szSetting = getModel()->GetLanguageString(IDS_TXT_PMINDELAY)+_T(" ")+getModel()->GetLanguageString(IDS_TXT_PMINDELAY2)+_T(":");
	m_iNumValues=0;
...
void CWndSubSettingsPminDelay::Draw()
{
	RECT rcCl;
	RECT rc;
	GetClientRect(&rcCl);
	...
	DrawText(hdcMem,getModel()->GetLanguageString(IDS_TXT_PMINDELAY),-1,&rc,DT_CENTER|DT_SINGLELINE|DT_TOP);
	rc.left = 220;  
	rc.top = 35;  
	rc.right  = 580;  
	rc.bottom = 500;
	//DrawText(hdcMem,m_szSetting,-1,&rc,DT_CENTER|DT_TOP|DT_SINGLELINE);
	DrawText(hdcMem,getModel()->GetLanguageString(IDS_TXT_PMINDELAY2),-1,&rc,DT_CENTER|DT_SINGLELINE|DT_TOP);
...
BOOL CWndSubSettingsPminDelay::PreTranslateMessage(MSG* pMsg) 
{
	switch(pMsg->message)
	{
	case WM_KEYDOWN:
		{
			...
			else if(pMsg->wParam==VK_DOWN)
			{
				...
				{
					if(m_iValue>MIN_TIME_PMINDELAY)
					{
						m_iValue-=STEP_TIME_PMINDELAY;
			...
			else if(pMsg->wParam==VK_UP)
			{
				if(m_pbtnValue->GetState()==BS_DOWN)
				{
					if(m_iValue<MAX_TIME_PMINDELAY)
					{
						m_iValue+=STEP_TIME_PMINDELAY;
...
void CWndSubSettingsPminDelay::OnDestroy()
{
	getModel()->getCONFIG()->SetCurPminAlarmDelay(m_iValue);
...
void CWndSubSettingsPminDelay::OnBnClickedValue()
{
...
LRESULT CWndSubSettingsPminDelay::WindowProc(UINT message, WPARAM wParam, LPARAM lParam )
{
	switch(message)
	{
...
void CWndSubSettingsPminDelay::OnTimer(UINT_PTR nIDEvent)
{
	CStringW strDate;
