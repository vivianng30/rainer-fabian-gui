// NumericFieldOXY.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CNumericFieldOXY, CWnd)
...
bool CNumericFieldOXY::drawData(bool bData, bool bFrames, bool bText, bool bLimits, bool bFlowmeter)
{
	...
	//###########################################################################
	SHORT iOxy=getModel()->getDATAHANDLER()->getMessureDataO2();
	...
	else
	{
		...
		iOxy=CTlsFloat::Round(((double)iOxy)/10, 0);
		if(iOxy<19 || getModel()->getDATAHANDLER()->isOxyHourglassRunning())
		{
			wsprintf(psz,_T("--"));
		}
		else
		...
		//wsprintf(psz,_T("%0.0f"),CTlsFloat::Round(((double)iOxy)/10, 1));
	}
