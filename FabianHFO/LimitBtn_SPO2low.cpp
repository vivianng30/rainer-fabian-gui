...
BEGIN_MESSAGE_MAP(CLimitBtn_SPO2low, CButton)
	//{{AFX_MSG_MAP(CLimitBtn_SPO2low)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_KEYDOWN()
...
void CLimitBtn_SPO2low::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if( m_bLMouseButtonDown )
	{
			...
			if(m_bDepressed)
			{
				...
				else
				{
					...
					if(GetParent() && m_bLimited==false)
						GetParent()->PostMessage(WM_SPO2LOW_LIMITEDLOW);
					m_bLimited=true;
				}
		...
		else if(nChar==m_kUp)	
		{
			if(m_bDepressed)
			{
				if(m_v.iValue<getModel()->getDATAHANDLER()->getPRICO_SPO2highRange())
				{
					int dTemp = m_v.iValue;
					dTemp++;

					if(dTemp>=getModel()->getDATAHANDLER()->getPRICO_SPO2highRange())
					{
						if(GetParent() && m_bLimited==false)
							GetParent()->PostMessage(WM_SPO2LOW_LIMITEDHIGH);
						m_bLimited=true;
					}
					else
					{
