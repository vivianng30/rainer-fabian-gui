// WndSubSettingsPatAlarmDelay.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CWndSubSettingsPatAlarmDelay, CWnd)
	ON_WM_DESTROY()
	ON_WM_TIMER()
...
void CWndSubSettingsPatAlarmDelay::Initialize()
{
	...
	m_iValue=getModel()->getCONFIG()->getCurPatientAlarmDelay();
	CStringW strVal;
	...
	m_szSetting = getModel()->GetLanguageString(IDS_TXT_ALAMDELAY);
	m_iNumValues=0;
...
void CWndSubSettingsPatAlarmDelay::Draw()
{
	...
	sz.Format(_T("%s: %d - %d %s"),
		...
		MIN_TIME_PATALARMDELAY,
		MAX_TIME_PATALARMDELAY,
	...
	case WM_KEYDOWN:
		{
			...
			else if(pMsg->wParam==VK_DOWN)
			{
				...
				{
					if(m_iValue>MIN_TIME_PATALARMDELAY)
					{
						m_iValue-=STEP_TIME_PATALARMDELAY;
			...
			else if(pMsg->wParam==VK_UP)
			{
				if(m_pbtnValue->GetState()==BS_DOWN)
				{
					if(m_iValue<MAX_TIME_PATALARMDELAY)
					{
						m_iValue+=STEP_TIME_PATALARMDELAY;
