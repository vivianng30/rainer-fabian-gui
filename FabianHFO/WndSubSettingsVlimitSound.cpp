// WndSubSettingsVlimitSound.cpp : implementation file
//

#include "stdafx.h"
#include "FabianHFO.h"
#include "WndSubSettingsVlimitSound.h"
...
IMPLEMENT_DYNAMIC(CWndSubSettingsVlimitSound, CWnd)

/**********************************************************************************************//**
 * Initializes a new instance of the CWndSubSettingsVlimitSound class
 *
 * \author	Rainer Kühner
 * \date	26.02.2018
 *
...
CWndSubSettingsVlimitSound::CWndSubSettingsVlimitSound(UINT viewFlag):
...
/**********************************************************************************************//**
 * Finalizes an instance of the CWndSubSettingsVlimitSound class
 *
 * \author	Rainer Kühner
 * \date	26.02.2018
 **************************************************************************************************/

CWndSubSettingsVlimitSound::~CWndSubSettingsVlimitSound()
{
	delete m_slider;
	m_slider=NULL;
}
...
BEGIN_MESSAGE_MAP(CWndSubSettingsVlimitSound, CWnd)
	ON_WM_DESTROY()
...
/**********************************************************************************************//**
 * CWndSubSettingsVlimitSound message handlers
 *
 * \author	Rainer Kühner
 * \date	26.02.2018
 **************************************************************************************************/

void CWndSubSettingsVlimitSound::Initialize()
{
	CClientDC dc(this);

	m_bVlimitBeep=getModel()->getCONFIG()->GetPlayVlimitSound();
...
void CWndSubSettingsVlimitSound::Draw()
{
	RECT rcCl;
	RECT rc;
	GetClientRect(&rcCl);
...
BOOL CWndSubSettingsVlimitSound::PreTranslateMessage(MSG* pMsg) 
{
	switch(pMsg->message)
	{
	case WM_KEYDOWN:
		{
			...
			else if(pMsg->wParam==VK_UP)
			{
				if(m_slider->GetPos()!=1)
				{
					m_slider->SetPos( 1 );
					m_bVlimitBeep=true;
					//getModel()->getCONFIG()->SetPlayVlimitSound(m_bVlimitBeep);
				}
				if(getModel()->isSafeTickCountDelayExpired(m_dwLastSetupTimer, 1000))
				{
					m_dwLastSetupTimer=GetTickCount();
...
void CWndSubSettingsVlimitSound::OnDestroy()
{
...
LRESULT CWndSubSettingsVlimitSound::OnMyMessage(WPARAM wParam, LPARAM lParam)
{
	switch(wParam)
	{
