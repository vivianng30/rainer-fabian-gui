// NumericFieldTRIGGER.cpp : implementation file
//
...
CNumericField(size)
{
	...
	{
		m_szName=getModel()->GetLanguageString(IDS_TXT_PRETRIGGER);
		m_szUnit=_T("");
	}
	else //trigger off
	{
		m_szName=getModel()->GetLanguageString(IDS_PARA_TRIGGER);
		m_szUnit=getModel()->GetLanguageString(IDS_TXT_NOTACTIVE);
	}
}
...
BEGIN_MESSAGE_MAP(CNumericFieldTRIGGER, CWnd)
...
bool CNumericFieldTRIGGER::drawData(bool, bool bFrames, bool bText, bool, bool)
{
	...
	//###########################################################################
	WORD iTrigger=getModel()->getDATAHANDLER()->getAVGMessureDataTrigger();
