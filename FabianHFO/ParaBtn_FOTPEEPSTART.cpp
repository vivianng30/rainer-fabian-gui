// ParaBtn_FOTPEEPSTART.cpp : implementation file
//
...
CParaBtn_FOTPEEPSTART::CParaBtn_FOTPEEPSTART(BTN btn, int nOffset,bool bScrollOver):
...
/**********************************************************************************************//**
 * Finalizes an instance of the CParaBtn_FOTPEEPSTART class
 *
 * \author	Rainer Kühner
 * \date	22.02.2018
 **************************************************************************************************/

CParaBtn_FOTPEEPSTART::~CParaBtn_FOTPEEPSTART()
{
}


BEGIN_MESSAGE_MAP(CParaBtn_FOTPEEPSTART, CButton)
	ON_WM_LBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_KEYDOWN()
...
/**********************************************************************************************//**
 * CParaBtn_FOTPEEPSTART message handlers
 *
 * \author	Rainer Kühner
 * \date	22.02.2018
 *
...
void CParaBtn_FOTPEEPSTART::Draw(int nState)
{
	if(!m_bDisableText)
	{
...
void CParaBtn_FOTPEEPSTART::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if( m_bLMouseButtonDown )
	{
		if(nChar==m_kDown)	
		{
			...
			if(GetParent())
				GetParent()->PostMessage(WM_FOT_PEEPSTART_CHANGED);
		}
		else if(nChar==m_kUp)	
		{
			if(m_bDepressed)
			{
				...
				//if(m_v.iValue<m_v.iUpperLimit)
				{
					...
					{
						iUpperLimit=getModel()->getDATAHANDLER()->PARADATA()->getFOTconv_PEEPENDPara()-getModel()->getCONFIG()->getDiffPEEPFOT();
						if(dTemp>iUpperLimit)
						{
							m_bLimited=true;

							if(GetParent())
								GetParent()->PostMessage(WM_FOT_PEEPSTART_LIMITED);
						}
						else
						{
							...
							if(dTemp>getModel()->getDATAHANDLER()->GetCurrentPeepMaxKey())
							{
								if(m_bKeyValueAccepted==false)
								{
									...
									m_bDrawKey=true;
									if(GetParent())
										GetParent()->PostMessage(WM_PEEPSTART_SETKEY,m_bKeyBeep);
									m_bKeyBeep=FALSE;
								}
								else
								{
					...
					{
						iUpperLimit=getModel()->getDATAHANDLER()->PARADATA()->GetPEEPMaxPara_IPPV();
						if(dTemp>iUpperLimit)
						{
							m_bEndOfRange = true;
						}
