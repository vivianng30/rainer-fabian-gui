// WndSubSettingsPpsvAbsolute.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CWndSubSettingsPpsvAbsolute, CWnd)

CWndSubSettingsPpsvAbsolute::CWndSubSettingsPpsvAbsolute(UINT viewFlag):
CWndSubSettings(viewFlag)
{
	m_bPPSVasDeltaPEEPValue=TRUE;
}

CWndSubSettingsPpsvAbsolute::~CWndSubSettingsPpsvAbsolute()
{
}


BEGIN_MESSAGE_MAP(CWndSubSettingsPpsvAbsolute, CWnd)
	ON_WM_DESTROY()
	//ON_MESSAGE(WM_BITMAPSLIDER_MOVING, OnMyMessage)
	ON_BN_CLICKED(IDC_BTN_SETUP_1, &CWndSubSettingsPpsvAbsolute::OnBnClicked1)
	ON_BN_CLICKED(IDC_BTN_SETUP_2, &CWndSubSettingsPpsvAbsolute::OnBnClicked2)
...
void CWndSubSettingsPpsvAbsolute::Initialize()
{
	...
	m_szSetting = getModel()->GetLanguageString(IDS_TXT_PPSV_SETTING);
	//m_szSetting = _T("Ppsv setting");
	m_iNumValues=5;

	m_pbtn1->ShowWindow(SW_SHOW);
	m_pbtn1->RefreshText(getModel()->GetLanguageString(IDS_TXT_PPSV_ABSOLUTE));

	m_pbtn2->ShowWindow(SW_SHOW);
	m_pbtn2->RefreshText(getModel()->GetLanguageString(IDS_TXT_PPSV_ABOVEPEEP));

	m_plBtn.AddTail(m_pbtn1);
	m_plBtn.AddTail(m_pbtn2);

	if(m_bPPSVasDeltaPEEPValue==TRUE)
		m_pbtn2->DrawOK(true);
	else
		m_pbtn1->DrawOK(true);
	...
	case WM_KEYDOWN:
		{
			if(pMsg->wParam==VK_SPACE)
			{
				...
				if(eState==BS_FOCUSED)
				{
					...
					SetPPSVasDeltaPEEPValue(m_iCurPosInBlock);
				}
