// WndSubSettingsBaroPressure.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CWndSubSettingsBaroPressure, CWnd)
	ON_WM_DESTROY()
	ON_WM_TIMER()
...
void CWndSubSettingsBaroPressure::Initialize()
{
	...
	m_iValue=getModel()->getCONFIG()->GetCO2BaroPressure();
	CStringW strVal;
	...
	m_szSetting = getModel()->GetLanguageString(IDS_TXT_BAROMETRICPRESSURE);
	m_iNumValues=0;
...
void CWndSubSettingsBaroPressure::Draw()
{
	...
	DrawText(hdcMem,szUnit,-1,&rc,DT_LEFT|DT_VCENTER|DT_SINGLELINE);
	...
	case WM_KEYDOWN:
		{
			...
			else if(pMsg->wParam==VK_DOWN)
			{
				...
				{
					if(m_iValue>MIN_BAROPRESSURE)
					{
						m_iValue-=STEP_BAROPRESSURE;
